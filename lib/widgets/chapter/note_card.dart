...
import 'package:familyacademyclient/widgets/common/app_button.dart';
...
class _NoteCardState extends State<NoteCard> {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius:
            BorderRadius.circular(ResponsiveValues.radiusFull(context)),
        border:
            Border.all(color: AppColors.telegramGreen.withValues(alpha: 0.3)),
      ),
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          ...
          Text(
            'Downloaded',
            style: AppTextStyles.caption(context).copyWith(
                color: AppColors.telegramGreen, fontWeight: FontWeight.w600),
          ),
        ],
      ),
    );
  ...
  void _showDeleteNoteDownloadDialog() {
    AppDialog.delete(
      ...
      message: 'Remove downloaded note "${widget.note.title}"?',
    ).then((confirmed) {
      if (confirmed == true) {
        ...
        children: [
          ...
          Expanded(
            child: hasFile && cachedPath != null && isPdf
                ...
                : _buildHtmlContent(context, note.content),
          ),
        ],
      ),
    );
